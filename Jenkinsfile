       ...
       agent {
        ...
        stages
        {
          stage('Build App')
          {
            steps
             {
              git branch: 'main', url: 'https://github.com/dhanbhan/openshift-jenkins-cicd.git'
              script {
          ...
          stage('Create Image Builder') {
            ...
            steps {
              script {
                openshift.withCluster() {
                  openshift.withProject() {
                    openshift.newBuild("--name=sample-app-jenkins-new", "--image-stream=openjdk18-openshift:1.16-1", "--binary=true")
                  }
                }
              }
            }
